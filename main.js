...
function decorateTable(tableEl) {
  ...
  // Columns 1+ are data columns (col 0 is the {calc} marker column)
  ...
  // ── Repurpose the {calc} th as the corner cell ──
  ...
  // ── Repurpose the first <td> in each data row as the row number ──
...
function hasCalcMarker(tableEl) {
  // Marker lives inside the table as the first <th> cell containing "{calc}".
  // This is reliable across all view modes — no sibling traversal needed.
...
class TableCalcPlugin extends obsidian.Plugin {
  constructor(...args) {
    super(...args);
    this.observers = [];
  }
  ...
  async onload() {
    ...
    // Watch every leaf for dynamically rendered tables (Live Preview)
    this.registerEvent(
    ...
    // Attach to whatever leaf is already open on load
    ...
    this.addCommand({
      ...
      checkCallback: (checking) => {
        ...
        if (!checking) {
          ...
          new obsidian.Notice('Table Calc: formulas evaluated');
        }
        return true;
      }
    });
  ...
  attachObserver(leaf) {
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      // Delay so surrounding DOM (including the {calc} sibling) is fully rendered
    ...
    // Process any tables already rendered in this leaf
    processEl(el);
  }
  ...
  onunload() {
    ...
    this.observers = [];
    // Clean up the marker so re-enabling works cleanly
