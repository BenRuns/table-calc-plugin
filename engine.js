...
function parseCells(lineFrom, lineText) {
  const cells = [];
  let i = 0;
  while (i < lineText.length) {
    if (lineText[i] === '|') {
      const contentStart = i + 1;
      const nextPipe = lineText.indexOf('|', contentStart);
      if (nextPipe === -1) break;
      const raw = lineText.slice(contentStart, nextPipe);
      const trimmed = raw.trim();
      const leadingSpaces = raw.length - raw.trimStart().length;
      cells.push({
        trimmed,
        from: lineFrom + contentStart + leadingSpaces,
        to:   lineFrom + contentStart + leadingSpaces + trimmed.length,
      });
      i = nextPipe;
    } else {
      i++;
...
// Takes objects with { text, from } (compatible with CM6 Line objects).
// Returns formulaCells: { from, to, formula, value }[]
// If the first header cell is "{calc}", that column is the row-number column
// and is excluded from the grid — so column A = second DOM column, B = third, etc.
function analyzeTable(lines) {
  const grid = [];
  const dataLines = [];
  ...
  for (const line of lines) {
    if (SEP_RE.test(line.text)) continue;
    const cells = parseCells(line.from, line.text);
    if (!headerSeen) {
      headerSeen = true;
      skipFirstCol = cells.length > 0 && cells[0].trimmed.toLowerCase() === '{calc}';
      continue;
    }
    const dataCells = skipFirstCol ? cells.slice(1) : cells;
    grid.push(dataCells.map(c => c.trimmed));
    dataLines.push(dataCells);
  }

  const formulaCells = [];
  dataLines.forEach((cells) => {
    cells.forEach((cell) => {
      if (!cell.trimmed.startsWith('=')) return;
      const value = evalFormula(cell.trimmed, grid);
      formulaCells.push({ from: cell.from, to: cell.to, formula: cell.trimmed, value });
    });
  });
...
module.exports = { parseRef, evalFormula, formatResult, parseCells, analyzeTable };
