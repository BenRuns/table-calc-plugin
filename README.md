...
- A text cell (non-blank, not a formula, not a valid number) referenced directly in a formula — `=A1`, `=A1+5`, `=SUM(A1:A3)`, `=AVERAGE(A1:A3)` — returns `#ERR` instead of silently treating the text as `0`. A **blank** cell is not text and still silently contributes `0`. `MIN`, `MAX`, `MEDIAN`, `PRODUCT`, `STDEV`, and `VAR` instead **exclude** non-numeric cells from the range entirely rather than erroring, since a phantom `0` would distort those results far more severely (e.g. zeroing out an entire `PRODUCT`, or pulling a `MEDIAN`/`STDEV` toward a value no cell actually contains), and excluding one bad cell out of many is safer than invalidating the whole result.
